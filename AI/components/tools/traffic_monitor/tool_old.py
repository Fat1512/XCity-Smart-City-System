    ...
    def call(
        ...
        with sv.VideoSink(output_video, video_info) as sink:
            for frame in sv.get_video_frames_generator(source_video):
                ...
                #     for tid in detection.tracker_id:
                #         tid_int = int(tid)
                #         trace = trace_annot.trace.get(tid_int)
                #         speedometer.update_with_trace(tid_int, trace)
                #         sp = speedometer.get_current_speed(tid_int)
                #         labels.append(f"#{tid_int} {sp} km/h")
